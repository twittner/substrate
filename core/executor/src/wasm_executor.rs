...
#[no_mangle] extern fn Z_envZ_ext_print_hexZ_vii(data: u32, len: u32) {
    /*
    if let Ok(hex) = thisFE.lock().unwrap().memory.get(data, len as usize) {
        println!("{}", HexDisplay::from(&hex));
    }
    Ok(())
    */
...
#[no_mangle] extern fn Z_envZ_ext_set_storageZ_viiii(key_data: u32, key_len: u32, value_data: u32, value_len: u32) {
    ...
    */
    ...
    /*
#[no_mangle] extern fn Z_envZ_ext_set_child_storage(storage_key_data: *const u8, storage_key_len: u32, key_data: *const u8, key_len: u32, value_data: *const u8, value_len: u32) {
    let storage_key = thisFE.lock().unwrap().memory.get(storage_key_data, storage_key_len as usize).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_set_child_storage"))?;
    let key = thisFE.lock().unwrap().memory.get(key_data, key_len as usize).map_err(|_| UserError("Invalid attempt to determine key in ext_set_child_storage"))?;
    let value = thisFE.lock().unwrap().memory.get(value_data, value_len as usize).map_err(|_| UserError("Invalid attempt to determine value in ext_set_child_storage"))?;
    if let Some(_preimage) = thisFE.lock().unwrap().hash_lookup.get(&key) {
        debug_trace!(
            target: "wasm-trace", "*** Setting child storage: {} -> %{} -> {}   [k={}]",
            ::primitives::hexdisplay::ascii_format(&storage_key),
    ...
    thisFE.lock().unwrap().ext.set_child_storage(storage_key, key, value);
    Ok(())
}
    */

    /*
#[no_mangle] extern fn Z_envZ_ext_clear_child_storage(storage_key_data: *const u8, storage_key_len: u32, key_data: *const u8, key_len: u32) {
    let storage_key = thisFE.lock().unwrap().memory.get(
        storage_key_data,
        storage_key_len as usize
    ).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_clear_child_storage"))?;
    let key = thisFE.lock().unwrap().memory.get(key_data, key_len as usize).map_err(|_| UserError("Invalid attempt to determine key in ext_clear_child_storage"))?;
    debug_trace!(target: "wasm-trace", "*** Clearing child storage: {} -> {}   [k={}]",
        ::primitives::hexdisplay::ascii_format(&storage_key),
        if let Some(_preimage) = thisFE.lock().unwrap().hash_lookup.get(&key) {
            format!("%{}", ::primitives::hexdisplay::ascii_format(&_preimage))
    ...
    thisFE.lock().unwrap().ext.clear_child_storage(&storage_key, &key);
    Ok(())
}
    */

#[no_mangle] extern fn Z_envZ_ext_clear_storageZ_vii(key_data: u32, key_len: u32) {
    /*
    let key = thisFE.lock().unwrap().memory.get(key_data, key_len as usize).map_err(|_| UserError("Invalid attempt to determine key in ext_clear_storage"))?;
    debug_trace!(target: "wasm-trace", "*** Clearing storage: {}   [k={}]",
        if let Some(_preimage) = thisFE.lock().unwrap().hash_lookup.get(&key) {
            format!("%{}", ::primitives::hexdisplay::ascii_format(&_preimage))
        } else {
    ...
    thisFE.lock().unwrap().ext.clear_storage(&key);
    Ok(())
    */
}

#[no_mangle] extern fn Z_envZ_ext_exists_storageZ_iii(key_data: u32, key_len: u32) -> u32 {
    /*
    let key = thisFE.lock().unwrap().memory.get(key_data, key_len as usize).map_err(|_| UserError("Invalid attempt to determine key in ext_exists_storage"))?;
    Ok(if thisFE.lock().unwrap().ext.exists_storage(&key) { 1 } else { 0 })
    */
    0u32
}

    /*
#[no_mangle] extern fn Z_envZ_ext_exists_child_storage(storage_key_data: *const u8, storage_key_len: u32, key_data: *const u8, key_len: u32) -> u32 {
    let storage_key = thisFE.lock().unwrap().memory.get(
        storage_key_data,
        storage_key_len as usize
    ).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_exists_child_storage"))?;
    let key = thisFE.lock().unwrap().memory.get(key_data, key_len as usize).map_err(|_| UserError("Invalid attempt to determine key in ext_exists_child_storage"))?;
    Ok(if thisFE.lock().unwrap().ext.exists_child_storage(&storage_key, &key) { 1 } else { 0 })
}
    */

#[no_mangle] extern fn Z_envZ_ext_clear_prefixZ_vii(prefix_data: u32, prefix_len: u32) {
    /*
    let prefix = thisFE.lock().unwrap().memory.get(prefix_data, prefix_len as usize).map_err(|_| UserError("Invalid attempt to determine prefix in ext_clear_prefix"))?;
    thisFE.lock().unwrap().ext.clear_prefix(&prefix);
    Ok(())
    */
}

    /*
#[no_mangle] extern fn Z_envZ_ext_kill_child_storage(storage_key_data: *const u8, storage_key_len: u32) {
    let storage_key = thisFE.lock().unwrap().memory.get(
        storage_key_data,
        storage_key_len as usize
    ).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_kill_child_storage"))?;
    thisFE.lock().unwrap().ext.kill_child_storage(&storage_key);
    Ok(())
}
    */
...
#[no_mangle] extern fn Z_envZ_ext_get_allocated_storageZ_iiii(key_data: u32, key_len: u32, written_out: u32) -> u32 {
    ...
    */
    ...
    /*
// return 0 and place u32::max_value() into written_out if no value exists for the key.
#[no_mangle] extern fn Z_envZ_ext_get_allocated_child_storage(storage_key_data: *const u8, storage_key_len: u32, key_data: *const u8, key_len: u32, written_out: *mut u32) -> *mut u8 {
    let storage_key = thisFE.lock().unwrap().memory.get(
        storage_key_data,
        storage_key_len as usize
    ).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_get_allocated_child_storage"))?;
    ...
    let maybe_value = thisFE.lock().unwrap().ext.child_storage(&storage_key, &key);
    ...
    if let Some(value) = maybe_value {
        let offset = thisFE.lock().unwrap().heap.allocate(value.len() as u32) as u32;
        thisFE.lock().unwrap().memory.set(offset, &value).map_err(|_| UserError("Invalid attempt to set memory in ext_get_allocated_child_storage"))?;
        thisFE.lock().unwrap().memory.write_primitive(written_out, value.len() as u32)
            .map_err(|_| UserError("Invalid attempt to write written_out in ext_get_allocated_child_storage"))?;
        Ok(offset)
    } else {
...
// return u32::max_value() if no value exists for the key.
#[no_mangle] extern fn Z_envZ_ext_get_storage_intoZ_iiiiii(key_data: u32, key_len: u32, value_data: u32, value_len: u32, value_offset: u32) -> u32 {
    /*
    let key = thisFE.lock().unwrap().memory.get(key_data, key_len as usize).map_err(|_| UserError("Invalid attempt to get key in ext_get_storage_into"))?;
    let maybe_value = thisFE.lock().unwrap().ext.storage(&key);
    debug_trace!(target: "wasm-trace", "*** Getting storage: {} == {}   [k={}]",
        if let Some(_preimage) = thisFE.lock().unwrap().hash_lookup.get(&key) {
            format!("%{}", ::primitives::hexdisplay::ascii_format(&_preimage))
    ...
    if let Some(value) = maybe_value {
        let value = &value[value_offset as usize..];
        let written = ::std::cmp::min(value_len as usize, value.len());
        thisFE.lock().unwrap().memory.set(value_data, &value[..written]).map_err(|_| UserError("Invalid attempt to set value in ext_get_storage_into"))?;
        Ok(written as u32)
    } else {
        Ok(u32::max_value())
    }
    */
    ...
    /*
// return u32::max_value() if no value exists for the key.
#[no_mangle] extern fn Z_envZ_ext_get_child_storage_into(storage_key_data: *const u8, storage_key_len: u32, key_data: *const u8, key_len: u32, value_data: *mut u8, value_len: u32, value_offset: u32) -> u32 {
    let storage_key = thisFE.lock().unwrap().memory.get(
        storage_key_data,
        storage_key_len as usize
    ).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_get_child_storage_into"))?;
    ...
    if let Some(value) = maybe_value {
        let value = &value[value_offset as usize..];
        let written = ::std::cmp::min(value_len as usize, value.len());
        thisFE.lock().unwrap().memory.set(value_data, &value[..written]).map_err(|_| UserError("Invalid attempt to set value in ext_get_child_storage_into"))?;
        Ok(written as u32)
    } else {
        Ok(u32::max_value())
    }
...
//Z_envZ_ext_storage_rootZ_vi(result: *mut u8) => {
#[no_mangle] extern fn Z_envZ_ext_storage_rootZ_vi(result: u32) {
    /*
    let r = thisFE.lock().unwrap().ext.storage_root();
    thisFE.lock().unwrap().memory.set(result, r.as_ref()).map_err(|_| UserError("Invalid attempt to set memory in ext_storage_root"))?;
    Ok(())
    */
}

    /*
#[no_mangle] extern fn Z_envZ_ext_child_storage_root(storage_key_data: *const u8, storage_key_len: u32, written_out: *mut u32) -> *mut u8 {
    let storage_key = thisFE.lock().unwrap().memory.get(storage_key_data, storage_key_len as usize).map_err(|_| UserError("Invalid attempt to determine storage_key in ext_child_storage_root"))?;
    let r = thisFE.lock().unwrap().ext.child_storage_root(&storage_key);
    if let Some(value) = r {
        let offset = thisFE.lock().unwrap().heap.allocate(value.len() as u32) as u32;
        thisFE.lock().unwrap().memory.set(offset, &value).map_err(|_| UserError("Invalid attempt to set memory in ext_child_storage_root"))?;
        thisFE.lock().unwrap().memory.write_primitive(written_out, value.len() as u32)
            .map_err(|_| UserError("Invalid attempt to write written_out in ext_child_storage_root"))?;
        Ok(offset)
    } else {
...
#[no_mangle] extern fn Z_envZ_ext_blake2_256_enumerated_trie_rootZ_viiii(values_data: u32, lens_data: u32, lens_len: u32, result: u32) {
    /*
    let values = (0..lens_len)
        .map(|i| thisFE.lock().unwrap().memory.read_primitive(lens_data + i * 4))
        .collect::<::std::result::Result<Vec<u32>, UserError>>()?
        .into_iter()
        .scan(0u32, |acc, v| { let o = *acc; *acc += v; Some((o, v)) })
        .map(|(offset, len)|
            thisFE.lock().unwrap().memory.get(values_data + offset, len as usize)
                .map_err(|_| UserError("Invalid attempt to get memory in ext_blake2_256_enumerated_trie_root"))
        )
        .collect::<::std::result::Result<Vec<_>, UserError>>()?;
    let r = ordered_trie_root::<Blake2Hasher, _, _>(values.into_iter());
    thisFE.lock().unwrap().memory.set(result, &r[..]).map_err(|_| UserError("Invalid attempt to set memory in ext_blake2_256_enumerated_trie_root"))?;
    Ok(())
    */
}

    /*
#[no_mangle] extern fn Z_envZ_ext_chain_id() -> u64 {
    Ok(thisFE.lock().unwrap().ext.chain_id())
}
    */

#[no_mangle] extern fn Z_envZ_ext_twox_128Z_viii(data: u32, len: u32, out: u32) {
    /*
    let result = if len == 0 {
        let hashed = twox_128(&[0u8; 0]);
        debug_trace!(target: "xxhash", "XXhash: '' -> {}", HexDisplay::from(&hashed));
        thisFE.lock().unwrap().hash_lookup.insert(hashed.to_vec(), vec![]);
        hashed
    } else {
        let key = thisFE.lock().unwrap().memory.get(data, len as usize).map_err(|_| UserError("Invalid attempt to get key in ext_twox_128"))?;
        let hashed_key = twox_128(&key);
        debug_trace!(target: "xxhash", "XXhash: {} -> {}",
            if let Ok(_skey) = ::std::str::from_utf8(&key) {
                _skey
        ...
        thisFE.lock().unwrap().hash_lookup.insert(hashed_key.to_vec(), key);
        hashed_key
    };

    thisFE.lock().unwrap().memory.set(out, &result).map_err(|_| UserError("Invalid attempt to set result in ext_twox_128"))?;
    Ok(())
    */
}

#[no_mangle] extern fn Z_envZ_ext_twox_256Z_viii(data: u32, len: u32, out: u32)  {
    /*
    let result = if len == 0 {
        twox_256(&[0u8; 0])
    } else {
        twox_256(&thisFE.lock().unwrap().memory.get(data, len as usize).map_err(|_| UserError("Invalid attempt to get data in ext_twox_256"))?)
    };
    thisFE.lock().unwrap().memory.set(out, &result).map_err(|_| UserError("Invalid attempt to set result in ext_twox_256"))?;
    Ok(())
    */
}

#[no_mangle] extern fn Z_envZ_ext_blake2_256Z_viii(data: u32, len: u32, out: u32) {
    /*
    let result = if len == 0 {
        blake2_256(&[0u8; 0])
    } else {
        blake2_256(&thisFE.lock().unwrap().memory.get(data, len as usize).map_err(|_| UserError("Invalid attempt to get data in ext_blake2_256"))?)
    };
    thisFE.lock().unwrap().memory.set(out, &result).map_err(|_| UserError("Invalid attempt to set result in ext_blake2_256"))?;
    Ok(())
    */
}

#[no_mangle] extern fn Z_envZ_ext_ed25519_verifyZ_iiiii(msg_data: u32, msg_len: u32, sig_data: u32, pubkey_data: u32) -> u32 {
    /*
    let mut sig = [0u8; 64];
    thisFE.lock().unwrap().memory.get_into(sig_data, &mut sig[..]).map_err(|_| UserError("Invalid attempt to get signature in ext_ed25519_verify"))?;
    let mut pubkey = [0u8; 32];
    thisFE.lock().unwrap().memory.get_into(pubkey_data, &mut pubkey[..]).map_err(|_| UserError("Invalid attempt to get pubkey in ext_ed25519_verify"))?;
    let msg = thisFE.lock().unwrap().memory.get(msg_data, msg_len as usize).map_err(|_| UserError("Invalid attempt to get message in ext_ed25519_verify"))?;
...
#[no_mangle] extern fn Z_envZ_ext_sandbox_instantiateZ_iiiiiii(
    ...
    /*
    let wasm = thisFE.lock().unwrap().memory.get(wasm_ptr, wasm_len as usize)
        .map_err(|_| UserError("OOB while ext_sandbox_instantiate: wasm"))?;
    let raw_env_def = thisFE.lock().unwrap().memory.get(imports_ptr, imports_len as usize)
        .map_err(|_| UserError("OOB while ext_sandbox_instantiate: imports"))?;

    // Extract a dispatch thunk from instance's table by the specified index.
    let dispatch_thunk = {
        let table = thisFE.lock().unwrap().table.as_ref()
            .ok_or_else(|| UserError("Runtime doesn't have a table; sandbox is unavailable"))?;
        table.get(dispatch_thunk_idx)
            .map_err(|_| UserError("dispatch_thunk_idx is out of the table bounds"))?
            .ok_or_else(|| UserError("dispatch_thunk_idx points on an empty table entry"))?
    ...
    let instance_idx_or_err_code =
        match sandbox::instantiate(thisFE.lock().unwrap(). dispatch_thunk, &wasm, &raw_env_def, state) {
            Ok(instance_idx) => instance_idx,
            Err(sandbox::InstantiationError::StartTrapped) => sandbox_primitives::ERR_EXECUTION,
            Err(_) => sandbox_primitives::ERR_MODULE,
        };
...
#[no_mangle] extern fn Z_envZ_ext_sandbox_instance_teardownZ_vi(instance_idx: u32) {
    /*
    thisFE.lock().unwrap().sandbox_store.instance_teardown(instance_idx)?;
    Ok(())
    */
}

#[no_mangle] extern fn Z_envZ_ext_sandbox_invokeZ_iiiiiiiii(instance_idx: u32, export_ptr: u32, export_len: u32, args_ptr: u32, args_len: u32, return_val_ptr: u32, return_val_len: u32, state: u32) -> u32 {
    /*
    use codec::{Decode, Encode};

    trace!(target: "sr-sandbox", "invoke, instance_idx={}", instance_idx);
    let export = thisFE.lock().unwrap().memory.get(export_ptr, export_len as usize)
        .map_err(|_| UserError("OOB while ext_sandbox_invoke: export"))
        .and_then(|b|
            String::from_utf8(b)
                .map_err(|_| UserError("export name should be a valid utf-8 sequence"))
    ...
    // Deserialize arguments and convert them into wasmi types.
    let serialized_args = thisFE.lock().unwrap().memory.get(args_ptr, args_len as usize)
        .map_err(|_| UserError("OOB while ext_sandbox_invoke: args"))?;
    let args = Vec::<sandbox_primitives::TypedValue>::decode(&mut &serialized_args[..])
        .ok_or_else(|| UserError("Can't decode serialized arguments for the invocation"))?
        .into_iter()
    ...
    let instance = thisFE.lock().unwrap().sandbox_store.instance(instance_idx)?;
    let result = instance.invoke(&export, &args, thisFE.lock().unwrap(). state);
...
#[no_mangle] extern fn Z_envZ_ext_sandbox_memory_newZ_iii(initial: u32, maximum: u32) -> u32 {
    /*
    let mem_idx = thisFE.lock().unwrap().sandbox_store.new_memory(initial, maximum)?;
    Ok(mem_idx)
    */
    0u32
}

#[no_mangle] extern fn Z_envZ_ext_sandbox_memory_getZ_iiiii(memory_idx: u32, offset: u32, buf_ptr: *mut u8, buf_len: u32) -> u32 {
    /*
    let sandboxed_memory = thisFE.lock().unwrap().sandbox_store.memory(memory_idx)?;
...
#[no_mangle] extern fn Z_envZ_ext_sandbox_memory_setZ_iiiii(memory_idx: u32, offset: u32, val_ptr: *const u8, val_len: u32) -> u32 {
    /*
    let sandboxed_memory = thisFE.lock().unwrap().sandbox_store.memory(memory_idx)?;
...
#[no_mangle] extern fn Z_envZ_ext_sandbox_memory_teardownZ_vi(memory_idx: u32) {
    /*
    thisFE.lock().unwrap().sandbox_store.memory_teardown(memory_idx)?;
    Ok(())
    */
}
